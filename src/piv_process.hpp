...
public:
    ...
    struct ProcessInfo
    {
        ...
        ~ProcessInfo()
        {
            if (hProcess != NULL && NtPiv::NtClose)
                NtPiv::NtClose(hProcess);
        }
    };
...
private:
    ...
    bool _get_base_info()
    {
        ...
        if (is_wow64(TRUE) == TRUE && is_wow64(FALSE) == FALSE)
        {
            ...
            if (NT_SUCCESS(NtPiv::NtWow64QueryInformationProcess64(ProInfo->hProcess, ProcessBasicInformation, &pbi, sizeof(pbi), NULL)))
            {
                NtPiv::PEB64 peb{0};
        ...
        else
        {
            ...
            if (NT_SUCCESS(NtPiv::NtQueryInformationProcess(ProInfo->hProcess, ProcessBasicInformation, &pbi, sizeof(pbi), NULL)))
            {
                NtPiv::PEB peb{0};
    ...
    {
        if (NtPiv::RtlAdjustPrivilege)
        {
            BOOLEAN Enabled;
            return NT_SUCCESS(NtPiv::RtlAdjustPrivilege(0x14, 1, 0, &Enabled));
        }
        else
        {
            HANDLE hToken;
    ...
    bool _query_virtual_memory(PVOID64 BaseAddress, ULONGLONG *RegionSize)
    {
        if (!ProInfo)
            return false;
        if (ProInfo->isWow64 && NtPiv::NtWow64QueryVirtualMemory64)
        {
            MEMORY_BASIC_INFORMATION64 mbi{0};
            if (NT_SUCCESS(NtPiv::NtWow64QueryVirtualMemory64(ProInfo->hProcess, BaseAddress, NtPiv::MemoryBasicInformation, &mbi, sizeof(mbi), NULL)))
            {
        ...
        else
        {
            ...
            if (NT_SUCCESS(NtPiv::NtQueryVirtualMemory(ProInfo->hProcess, reinterpret_cast<PVOID>(BaseAddress), NtPiv::MemoryBasicInformation, &mbi, sizeof(mbi), NULL)))
            {
                *RegionSize = mbi.RegionSize;
                return true;
            }
...
public:
    ...
    bool open_process(DWORD process_id, DWORD desired_access, BOOL enable_debug = FALSE)
    {
        if (NtPiv::LoadNt())
        {
            ...
            if (NT_SUCCESS(NtPiv::NtOpenProcess(&ProInfo->hProcess, desired_access, &oa, &cid)) && _get_base_info())
                return true;
        }
     ...
     * @return 返回是否成功
     */
    inline bool get_memory_info(PROCESS_MEMORY_COUNTERS *pMemCounters) noexcept
    {
        if (ProInfo)
            return ::GetProcessMemoryInfo(ProInfo->hProcess, pMemCounters, sizeof(PROCESS_MEMORY_COUNTERS));
        return false;
    }
    ...
    inline BOOL get_dep_policy(DWORD &flags, BOOL &is_permanet) const noexcept
    {
        flags = 0;
        is_permanet = FALSE;
        if (ProInfo && NtPiv::GetProcessDEPPolicy)
            return NtPiv::GetProcessDEPPolicy(ProInfo->hProcess, &flags, &is_permanet);
        return FALSE;
    }
    ...
    {
        BOOL result = FALSE;
        if (NtPiv::IsWow64Process)
        {
            if (currentProcess)
            {
                NtPiv::IsWow64Process(::GetCurrentProcess(), &result);
            }
            else if (ProInfo)
            {
                NtPiv::IsWow64Process(ProInfo->hProcess, &result);
            }
        }
        return result;
    }
     ...
     */
    inline CVolString get_mapped_filename(void *vmem_address) noexcept
    {
        if (ProInfo)
        {
            CVolString FileName;
            FileName.m_mem.Alloc((MAX_PATH + 1) * 2, TRUE);
            if (::GetMappedFileNameW(ProInfo->hProcess, vmem_address, const_cast<wchar_t *>(FileName.GetText()), MAX_PATH) > 0)
                return FileName;
        }
        return _CT("");
    }
    ...
    inline CVolString get_image_filename() noexcept
    {
        if (ProInfo)
        {
            ...
            if (::GetProcessImageFileNameW(ProInfo->hProcess, const_cast<wchar_t *>(ImageFilename.GetText()), MAX_PATH) > 0)
    ...
    inline bool create_remote_thread(void *func_ptr, void *parm_ptr, BOOL is_wait) noexcept
    {
        ...
        {
            HANDLE hThread = NULL;
            if (NtPiv::NtCreateThreadEx)
                NtPiv::NtCreateThreadEx(&hThread, 0x1FFFFF, NULL, ProInfo->hProcess, func_ptr, parm_ptr, 0, 0, 0, 0, NULL);
            else
    ...
    inline PVOID64 virtual_alloc(PVOID64 address, uint64_t men_size, DWORD allocation_type, DWORD protect) noexcept
    {
        ...
        {
            NTSTATUS status;
            if (ProInfo->isWow64 && NtPiv::NtWow64AllocateVirtualMemory64)
                status = NtPiv::NtWow64AllocateVirtualMemory64(ProInfo->hProcess, &address, 0, &men_size, allocation_type, protect);
            else
                status = NtPiv::NtAllocateVirtualMemory(ProInfo->hProcess, reinterpret_cast<PVOID *>(&address), 0, reinterpret_cast<PSIZE_T>(&men_size), allocation_type, protect);
    ...
    {
        if (ProInfo)
            return NT_SUCCESS(NtPiv::NtFreeVirtualMemory(ProInfo->hProcess, &address, reinterpret_cast<PSIZE_T>(&men_size), free_type));
        return false;
    }
    ...
    inline int32_t modify_memory_protect(void *address, size_t mem_size, DWORD new_protect) noexcept
    {
        ...
        {
            DWORD OldProtect;
            if (NT_SUCCESS(NtPiv::NtProtectVirtualMemory(ProInfo->hProcess, &address, reinterpret_cast<PSIZE_T>(&mem_size), new_protect, &OldProtect)))
    ...
    inline bool read_memory(void *write_address, PVOID64 read_address, uint64_t read_size) noexcept
    {
        if (ProInfo)
        {
            if (ProInfo->isWow64 && NtPiv::NtWow64ReadVirtualMemory64)
                return NT_SUCCESS(NtPiv::NtWow64ReadVirtualMemory64(ProInfo->hProcess, read_address, write_address, read_size, NULL));
            else
                return NT_SUCCESS(NtPiv::NtReadVirtualMemory(ProInfo->hProcess, reinterpret_cast<void *>(read_address), write_address, static_cast<SIZE_T>(read_size), NULL));
        }
        return false;
    }
    ...
    inline bool write_memory(PVOID64 write_address, const void *data_address, size_t data_size) noexcept
    {
        ...
        if (ProInfo)
        {
            if (ProInfo->isWow64 && !NtPiv::NtWow64WriteVirtualMemory64)
            {
                result = NT_SUCCESS(NtPiv::NtWow64WriteVirtualMemory64(ProInfo->hProcess, write_address, data_address, data_size, NULL));
            }
            else
            {
                DWORD dwOldProtect = 0;
                if (NT_SUCCESS(NtPiv::NtProtectVirtualMemory(ProInfo->hProcess, reinterpret_cast<PVOID *>(&write_address), reinterpret_cast<PSIZE_T>(&data_size), PAGE_EXECUTE_READWRITE, &dwOldProtect)))
                {
                    result = NT_SUCCESS(NtPiv::NtWriteVirtualMemory(ProInfo->hProcess, reinterpret_cast<PVOID>(write_address), data_address, data_size, NULL));
                    NtPiv::NtProtectVirtualMemory(ProInfo->hProcess, reinterpret_cast<PVOID *>(&write_address), reinterpret_cast<PSIZE_T>(&data_size), dwOldProtect, NULL);
                }
            }
        }
        return result;
