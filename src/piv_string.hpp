...
inline std::basic_string<CharT>& assign(std::basic_string<CharT>& lhs, V rhs, typename std::enable_if<std::is_integral<V>::value, V>::type* = nullptr)
{
    return piv::edit::assign_int(lhs, rhs);
}
...
inline typename std::enable_if<std::is_same<typename std::remove_reference<T>::type, std::string>::value, T>::type
push_back(T&& lhs, C ch)
{
    lhs.push_back(static_cast<char>(ch));
    return std::forward<T>(lhs);
}

template <typename T, typename C, typename... Args>
inline typename std::enable_if<std::is_same<typename std::remove_reference<T>::type, std::string>::value, T>::type
push_back(T&& lhs, C ch, Args&&... args)
{
    lhs.push_back(static_cast<char>(ch));
...
push_back(T&& lhs, C ch)
{
    lhs.push_back(static_cast<wchar_t>(ch));
    return std::forward<T>(lhs);
}
...
template <typename CharT, typename V>
inline std::basic_string<CharT>& append(std::basic_string<CharT>& lhs, V rhs, typename std::enable_if<std::is_integral<V>::value, V>::type* = nullptr)
{
    return piv::edit::append_int(lhs, rhs);
}
 ...
 */
template <typename T, typename CharT>
typename std::enable_if<std::is_same<typename std::remove_reference<T>::type, std::string>::value, T>::type inline append_hex(T&& lhs, const piv::basic_string_view<CharT>& hexstr, size_t count = -1)
{
...
struct ci_equal_to<std::basic_string<CharT>>
{
    bool operator()(const std::basic_string<CharT>& lhs, const std::basic_string<CharT>& rhs) const
    {
        return ::piv::edit::iequals(lhs, rhs);
    }
};
