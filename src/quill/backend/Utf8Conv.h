...
QUILL_NODISCARD QUILL_EXPORT QUILL_ATTRIBUTE_USED inline std::string utf8_encode(std::byte const* data, size_t wide_str_len)
{
...
#ifdef SIMDUTF_H
  size_t size_needed = simdutf::utf8_length_from_utf16le(reinterpret_cast<const char16_t *>(data), wide_str_len);
  ...
  std::string ret_val(size_needed, '\0');
  simdutf::convert_valid_utf16le_to_utf8(reinterpret_cast<const char16_t *>(data), wide_str_len, &ret_val[0]);
  return ret_val;
#else
