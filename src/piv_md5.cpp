...
static wchar_t hb2hex(unsigned char hb) {
    hb = hb & 0xF;
...
CVolString GetFileMd5(FILE* file, BOOL upper){
	...
	CVolString res;
	res.SetNumAlignChars(32);
	if (upper) {
		for(size_t i = 0; i < 16; ++ i) {
			res.AddChar(hb2HEX(out[i] >> 4));
			res.AddChar(hb2HEX(out[i]));
		}
	} else {
		for(size_t i = 0; i < 16; ++ i) {
			res.AddChar(hb2hex(out[i] >> 4));
...
CVolString GetDataMd5(CVolMem &dat, BOOL upper) {
	return GetDataMd5(dat.GetPtr(), dat.GetSize(), upper);
}
...
CVolString GetDataMd5sum6(CVolMem &dat) {
	return GetDataMd5sum6(dat.GetPtr(), dat.GetSize());
}
