...
inline void MarshalTo(const ULID& ulid, CharT dst[26]) {
	// 10 byte timestamp
 ...
 * */
template <typename CharT>
inline void UnmarshalFrom(const CharT str[26], ULID& ulid) {
	// timestamp
...
inline ULID Unmarshal(const std::string& str) {
	ULID ulid;
	UnmarshalFrom<char>(str.c_str(), ulid);
	return ulid;
}
