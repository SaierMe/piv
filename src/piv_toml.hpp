...
{
    ::toml::parse_result result = ::toml::parse_file(PivS2UV{file_path}.sv);
    if (result)
    {
...
{
    ::toml::parse_result result = ::toml::parse(PivS2UV{doc}.sv, PivS2UV{source_path}.sv);
    if (result)
    {
        tbl = std::move(result.table());
        return true;
...
{
    auto val = tbl.get_as<R>(PivS2UV{key}.sv);
    if (val)
        return **val;
    return R{};
}
...
{
    auto val = tbl.get_as<::toml::table>(PivS2UV{key}.sv);
    if (val)
...
{
    auto val = tbl.get_as<::toml::array>(PivS2UV{key}.sv);
    if (val)
        return *val;
    return def_val;
}
