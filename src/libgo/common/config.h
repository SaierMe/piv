#ifndef LIBGO_COMMON_CONFIG_H
#define LIBGO_COMMON_CONFIG_H

#ifndef LIBGO_ENABLE_HOOK
#define LIBGO_ENABLE_HOOK 1
#endif
#ifndef LIBGO_ENABLE_DEBUGGER
#define LIBGO_ENABLE_DEBUGGER 0
#endif
...
# define LIBGO_INLINE __attribute__ ((always_inline)) inline 
#else
# define LIBGO_INLINE inline
#endif
...
#define LIBGO_DebugPrint(type, fmt, ...) \
    do { \
...
struct CoroutineOptions
{
    ...
    LIBGO_INLINE static CoroutineOptions& getInstance()
    {
...
static const char* BaseFile(const char* file)
{
...
static unsigned long NativeThreadID()
{
#if defined(LIBGO_SYS_Unix)
...
#endif
static std::string Format(const char* fmt, ...) GCC_FORMAT_CHECK
{
    va_list ap;
    va_start(ap, fmt);
    char buf[4096];
...
static std::string P(const char* fmt, ...) GCC_FORMAT_CHECK
{
    va_list ap;
    va_start(ap, fmt);
    char buf[4096];
