#ifndef LIBGO_COMMON_TS_QUEUE_H
#define LIBGO_COMMON_TS_QUEUE_H
#include "config.h"
#include "util.h"
#include "spinlock.h"
...
struct TSQueueHook
{
    ...
    LIBGO_INLINE void link(TSQueueHook* theNext) {
        assert(next == nullptr);
    ...
    LIBGO_INLINE void unlink(TSQueueHook* theNext) {
        assert(next == theNext);
...
public:
    ...
    LIBGO_INLINE bool empty() const { return head_ == nullptr; }
...
//        printf("SList.erase done\n");
    ...
    LIBGO_INLINE TSQueueHook* head() { return head_; }
    LIBGO_INLINE TSQueueHook* tail() { return tail_; }
};
...
public:
    ...
    LIBGO_INLINE lock_t& LockRef() {
        return *lock_;
    }

    LIBGO_INLINE void front(T*& out)
    {
        LockGuard lock(*lock_);
        out = (T*)head_->next;
    ...
    LIBGO_INLINE void next(T* ptr, T*& out)
    {
        LockGuard lock(*lock_);
        nextWithoutLock(ptr, out);
    }

    LIBGO_INLINE void nextWithoutLock(T* ptr, T*& out)
    {
        out = (T*)ptr->next;
        if (out) out->check_ = check_;
    }

    LIBGO_INLINE bool empty()
    {
        LockGuard lock(*lock_);
        return !count_;
    }

    LIBGO_INLINE bool emptyUnsafe()
    {
        return !count_;
    }

    LIBGO_INLINE std::size_t size()
    {
        LockGuard lock(*lock_);
        return count_;
    }
    ...
    LIBGO_INLINE void push(SList<T> && elements)
    {
    ...
    LIBGO_INLINE void pushWithoutLock(SList<T> && elements)
    {
        if (elements.empty()) return ;
    ...
    // O(n), 慎用.
    LIBGO_INLINE SList<T> pop_front(uint32_t n)
    {
    ...
    // O(n), 慎用.
    LIBGO_INLINE SList<T> pop_back(uint32_t n)
    {
        if (head_ == tail_) return SList<T>();
        LockGuard lock(*lock_);
        return pop_backWithoutLock(n);
    }
    LIBGO_INLINE SList<T> pop_backWithoutLock(uint32_t n)
    {
        if (head_ == tail_) return SList<T>();
    ...
    LIBGO_INLINE SList<T> pop_allWithoutLock()
    {
        if (head_ == tail_) return SList<T>();
        TSQueueHook* first = head_->next;
        TSQueueHook* last = tail_;
    ...
    LIBGO_INLINE bool erase(T* hook, bool check = false)
    {
        LockGuard lock(*lock_);
        return eraseWithoutLock(hook, check);
    }

    LIBGO_INLINE bool eraseWithoutLock(T* hook, bool check = false, bool refCount = true)
    {
    ...
    LIBGO_INLINE size_t pushWithoutLock(T* element, bool refCount = true)
    {
        ...
        hook->next = nullptr;
        hook->check_ = check_;
        LIBGO_DebugPrint(dbg_suspend, "pushWithoutLock before this=0x%p size=%zu", (void*)this, count_);
        ++ count_;
        if (refCount)
    ...
    LIBGO_INLINE size_t push(T* element)
    {
        LockGuard lock(*lock_);
        return pushWithoutLock(element);
    }

    LIBGO_INLINE void AssertLink()
    {
#if LIBGO_DEBUG
        LockGuard lock(*lock_);
