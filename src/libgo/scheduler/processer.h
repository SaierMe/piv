#ifndef LIBGO_SCHEDULER_PROCESSER_H
#define LIBGO_SCHEDULER_PROCESSER_H
...
public:
    LIBGO_INLINE int Id() { return id_; }
    ...
    // 协程切出
    LIBGO_INLINE static void StaticCoYield();
...
private:
    ...
    // 调度线程会尽量分配协程过来
    LIBGO_INLINE bool IsWaiting() { return waiting_; }
...
LIBGO_INLINE void Processer::StaticCoYield()
{
    auto proc = GetCurrentProcesser();
    if (proc) proc->CoYield();
}

LIBGO_INLINE void Processer::CoYield()
{
...
#if LIBGO_ENABLE_DEBUGGER
    LIBGO_DebugPrint(dbg_yield, "yield task(%s) state = %s", tk->DebugInfo(), GetTaskStateName(tk->state_));
...
#endif // LIBGO_SCHEDULER_PROCESSER_H
