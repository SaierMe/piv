#ifndef LIBGO_ROUTINE_SYNC_MUTEX_H
#define LIBGO_ROUTINE_SYNC_MUTEX_H
#include "rutex.h"
...
public:
    ...
    void unlock()
    {
        LIBGO_RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s",
        ...
        if (prev == LIBGO_ROUTINE_SYNC_MUTEX_LOCKED) {
            LIBGO_RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | not contended",
                    id(), rutex_.id(), __func__);
            return ;
        }
        ...
        (void)res;
        LIBGO_RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | contended | notify_one return %d",
...
public:
    ...
    {
        if (try_lock()) {
            LIBGO_RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | try_lock success",
                    id(), rutex_.id(), __func__, !!abstime);
            return true;
        }
        
        int res = lock_contended(abstime);
        LIBGO_RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | lock_contended %s",
                    id(), rutex_.id(), __func__, !!abstime,
    ...
    inline int lock_contended(const std::chrono::time_point<_Clock, _Duration> * abstime) {
        ...
        while (whole->exchange(LIBGO_ROUTINE_SYNC_MUTEX_CONTENDED) & LIBGO_ROUTINE_SYNC_MUTEX_LOCKED) {
            LIBGO_RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | begin wait_until",
                    id(), rutex_.id(), __func__, !!abstime);

            int res = rutex_.wait_until(LIBGO_ROUTINE_SYNC_MUTEX_CONTENDED, abstime);
            
            LIBGO_RS_DBG(dbg_mutex, "mutex=%ld rutex=%ld | %s | abstime=%d | wait_until return %d",
...
#endif // LIBGO_ROUTINE_SYNC_MUTEX_H
