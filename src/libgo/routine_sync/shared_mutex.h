#ifndef LIBGO_ROUTINE_SYNC_SHARED_MUTEX_H
#define LIBGO_ROUTINE_SYNC_SHARED_MUTEX_H
#include "rutex.h"
...
public:
    ...
    bool try_lock()
    {
        ...
        if (expect != 0 && expect != -2) {
            LIBGO_RS_DBG(dbg_mutex, "shared_mutex=%ld(r=%ld w=%ld) | %s |"
...
public:
    ...
    {
        LIBGO_RS_DBG(dbg_mutex, "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d | enter",
        ...
        for (;;) {
            ...
            LIBGO_RS_DBG(dbg_mutex, "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d |"
            ...
            LIBGO_RS_DBG(dbg_mutex, "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d | begin wait_until",
                    id(), readers_.id(), writers_.id(), __func__, !!abstime);

            auto res = writers_.wait_until(v, abstime);
            if (res == RutexBase::rutex_wait_return_etimeout) {
                LIBGO_RS_DBG(dbg_mutex, "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d | wait_until return timeout",
                        id(), readers_.id(), writers_.id(), __func__, !!abstime);
                return false;
            }

            LIBGO_RS_DBG(dbg_mutex, "shared_mutex=%ld(r=%ld w=%ld) | %s | abstime=%d | wait_until return success",
                    id(), readers_.id(), writers_.id(), __func__, !!abstime);
        }
    }
...
#endif // LIBGO_ROUTINE_SYNC_SHARED_MUTEX_H
