#ifndef LIBGO_ROUTINE_SYNC_CHANNEL_H
#define LIBGO_ROUTINE_SYNC_CHANNEL_H
...
public:
    ...
    {
        LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | ptr(t)=0x%p | isWait=%d | abstime=%d | closed=%d | popWaiting_=%p | pushWaiting_=%p",
        ...
        if (pushWaiting_ && pushWaiting_ != (ConditionVariable*)kClosedWaiting) {   // 有push协程在等待, 读出来 & 清理
            ...
            LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | match pop success | notify peer",
                id(), __func__);
        ...
        if (!isWait) {
            LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | not match && not wait | return false",
                id(), __func__);
            return false;
        }
        ...
        LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | begin wait matched",
            id(), __func__);
        ...
        LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | waked | closed=%d | changed=%d | ok=%d",
            ...
            const std::chrono::time_point<_Clock, _Duration>* abstime)
    {
        ...
        if (!isWait) {
            LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | pop contended && not wait | return false",
                id(), __func__);
            return false;
        }

        // 有其他pop在等待, 进pop队列
        LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | begin wait at pop queued",
            id(), __func__);

        auto p = [this]{ return !popWaiting_; };
        while (!p()) {
            if (popCv_.wait_until_impl(lock, abstime) == std::cv_status::timeout) {
                LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | waked | pop busy | timeout",
                    id(), __func__);
                return false;
            }
        }

        LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | waked | closed=%d | pop idle",
            id(), __func__, closed_);
            ...
            std::unique_lock<Mutex> & lock)
    {
        ...
        if (popWaiting_ && popWaiting_ != (ConditionVariable*)kClosedWaiting) {   // 有pop协程在等待, 写入 & 清理
            *popQ_ = t;
            ...
            LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | match push success | notify peer",
                id(), __func__);
            ...
            const std::chrono::time_point<_Clock, _Duration>* abstime)
    {
        ...
        if (!isWait) {
            LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | push contended && not wait | return false",
                id(), __func__);
            return false;
        }

        // 有其他push在等待, 进push队列
        LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | begin wait at push queued",
            id(), __func__);

        auto p = [this]{ return !pushQ_; };
        while (!p()) {
            if (pushCv_.wait_until_impl(lock, abstime) == std::cv_status::timeout) {
                LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | waked | push busy | timeout",
                    id(), __func__);
                return false;
            }
        }

        LIBGO_RS_DBG(dbg_channel, "channel=%ld | %s | waked | closed=%d | push idle",
            id(), __func__, closed_);
    ...
    void impl_with_signal_close(std::unique_lock<Mutex> & lock)
    {
        ...
        (void)push_wakeup;
        (void)pop_wakeup;
        LIBGO_RS_DBG(dbg_channel, "channel(...)=%ld | %s | no-cap branch | push-wakeup=%ld | pop-wakeup=%ld",
...
public:
    ...
    void close()
    {
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | cap=%lu | size=%lu | push-wakeup=%ld | pop-wakeup=%ld",
...
private:
            ...
            const std::chrono::time_point<_Clock, _Duration>* abstime)
    {
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | ptr(t)=0x%p | isWait=%d | abstime=%d | closed=%d | cap=%lu | size=%lu",
        ...
        if (q_.size() < cap_) {
            ...
            LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | ptr(t)=0x%p | push success | notify",
                id(), __func__, (void*)&t);
        ...
        if (!isWait) {
            LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | not match && not wait | return false",
                id(), __func__);
            return false;
        }
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | begin wait",
                id(), __func__);

        std::cv_status status = pushCv_.wait_until_p(lock, abstime, p);

        LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | waked | status=%s | closed=%d",
                id(), __func__,
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | ptr(t)=0x%p | push success after wait | size=%lu | notify",
            ...
            const std::chrono::time_point<_Clock, _Duration>* abstime)
    {
        ...
        if (!q_.empty()) {
            ...
            LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | ptr(t)=0x%p | pop success | notify",
                id(), __func__, (void*)&t);
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(queue)=%ld | %s | ptr(t)=0x%p | pop success after wait | size=%lu | notify",
                id(), __func__, (void*)&t, q_.size());
...
public:
    ...
    void close()
    {
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | cap=%lu | size=%lu | push-wakeup=%ld | pop-wakeup=%ld",
...
private:
            ...
            const std::chrono::time_point<_Clock, _Duration>* abstime)
    {
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | isWait=%d | abstime=%d | closed=%d | cap=%lu | size=%lu",
        ...
        if (count_ < cap_) {
            ...
            LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | push success | notify",
                id(), __func__);
        ...
        if (!isWait) {
            LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | not match && not wait | return false",
                id(), __func__);
            return false;
        }
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | begin wait",
                id(), __func__);

        std::cv_status status = pushCv_.wait_until_p(lock, abstime, p);

        LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | waked | status=%s | closed=%d",
                id(), __func__,
                (status == std::cv_status::timeout) ? "timeout" : "no_timeout", closed_);
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | push success after wait | size=%lu | notify",
                id(), __func__, count_);
            ...
            const std::chrono::time_point<_Clock, _Duration>* abstime)
    {
        ...
        if (count_ > 0) {
            ...
            LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | pop success | notify",
                id(), __func__);
            return true;
        }
        ...
        LIBGO_RS_DBG(dbg_channel, "channel(void)=%ld | %s | pop success after wait | size=%lu | notify",
                id(), __func__, count_);
...
#endif // LIBGO_ROUTINE_SYNC_CHANNEL_H
