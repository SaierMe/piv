#ifndef LIBGO_ROUTINE_SYNC_RUTEX_H
#define LIBGO_ROUTINE_SYNC_RUTEX_H
#include "switcher.h"
#include <atomic>
#include <mutex>
#include <memory>
...
public:
    ...
    void wake_by_timer() {
        ...
        if (!lock.try_lock()) {
            LIBGO_RS_DBG(dbg_rutex, "rw=%ld | %s | try_lock=0",
                    id(), __func__);
            return ;
        }
        
        if (!safe_unlink()) {
            LIBGO_RS_DBG(dbg_rutex, "rw=%ld | %s | safe_unlink failed",
                    id(), __func__);
            return ;
        }

        if (wake(waiter_state_timeout)) {
            LIBGO_RS_DBG(dbg_rutex, "rw=%ld | %s | wake success",
                    id(), __func__);
            return ;
        }

        if (!waked_.load(std::memory_order_relaxed)) {
            LIBGO_RS_DBG(dbg_rutex, "rw=%ld | %s | wake failed, retry delay=%d",
            ...
            LIBGO_RS_DBG(dbg_rutex, "rw=%ld | %s | wake failed, needn't retry.",
                    id(), __func__);
        }
    }
...
public:
            ...
            const std::chrono::time_point<_Clock, _Duration> * abstime)
    {
        if (base_t::value()->load(std::memory_order_relaxed) != expectValue) {
            ...
            LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | check fail | %s",
        ...
        if (!switcher->valid()) {
            ...
            if (RoutineSyncPolicy::isInPThread()) {
                ...
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | switcher invalid && not in pthread | %s",
                    id(), __func__, !!abstime, etos(rutex_wait_return_ewouldblock));
                return rutex_wait_return_ewouldblock;
            }
        }

        RutexWaiter* rw = new RutexWaiter(*switcher);

        {
            std::unique_lock<std::mutex> lock(mtx_);
            if (base_t::value()->load(std::memory_order_relaxed) != expectValue) {
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d rw=%ld | check fail in locked | %s",
            ...
            switch (state) {
                case RutexWaiter::waiter_state_interrupted:
                    LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d rw=%ld | state eintr in locked | %s",
                        id(), __func__, !!abstime, rw->id(), etos(rutex_wait_return_eintr));
                    return rutex_wait_return_eintr;

                case RutexWaiter::waiter_state_ready:
                    LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d rw=%ld | state ready in locked | %s",
        ...
        LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | rw=%ld | sleep",
            id(), __func__, !!abstime, rw->id());
        rw->sleep(abstime);

        LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d | rw=%ld | waked | begin join",
            id(), __func__, !!abstime, rw->id());
        rw->join();
        ...
        switch (state) {
            ...
            case RutexWaiter::waiter_state_interrupted:
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d rw=%ld | joined state none or intr | %s",
                        id(), __func__, !!abstime, rw->id(), etos(rutex_wait_return_eintr));
                return rutex_wait_return_eintr;

            case RutexWaiter::waiter_state_timeout:
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d rw=%ld | joined state timeout | %s",
                        id(), __func__, !!abstime, rw->id(), etos(rutex_wait_return_etimeout));
                return rutex_wait_return_etimeout;
        }

        LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | abstime=%d rw=%ld | joined state ready | %s",
            id(), __func__, !!abstime, rw->id(), etos(rutex_wait_return_success));
        return rutex_wait_return_success;
    }
    ...
    int notify_one()
    {
        for (;;) {
            ...
            if (!rw) {
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | empty list | return 0",
                    id(), __func__);
                return 0;
            }
            ...
            LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld try_lock=%d",
            ...
            lock.unlock();
            if (rw->wake()) {
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld wake success | return 1",
                    id(), __func__, rw->id());
                return 1;
            } else {
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | rw=%ld wake failed",
                    id(), __func__, rw->id());
            }
            delete rw;
        }
    ...
    {
        LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld",
        ...
        for (;;n++) {
            RutexWaiter *rw = static_cast<RutexWaiter *>(waiters_.front());
            if (!rw) {
                LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld | done n=%d",
                    id(), __func__, other->id(), n);
                return n;
            }

            LIBGO_RS_DBG(dbg_rutex, "rutex=%ld | %s | other=%ld | rw=%ld",
...
#endif // LIBGO_ROUTINE_SYNC_RUTEX_H
