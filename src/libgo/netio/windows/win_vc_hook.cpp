...
namespace co {
    ...
    static VOID WINAPI hook_Sleep(_In_ DWORD dwMilliseconds)
    {
        ...
        //LIBGO_DebugPrint(dbg_hook, "task(%s) Hook Sleep(dwMilliseconds=%lu).", tk->DebugInfo(), dwMilliseconds);
        if (!tk) {
    ...
    {
        //Task *tk = Processer::GetCurrentTask();
        //LIBGO_DebugPrint(dbg_hook, "task(%s) safe_select(nfds=%d, rfds=%p, wfds=%p, efds=%p).",
        ...
        )
    {
        ...
        Task *tk = Processer::GetCurrentTask();
        LIBGO_DebugPrint(dbg_hook, "task(%s) Hook select(nfds=%d, rfds=%p, wfds=%p, efds=%p, timeout=%d).", 
        ...
        Args && ... args)
    {
        ...
        LIBGO_DebugPrint(dbg_hook, "task(%s) Hook %s(s=%d)(nonblocking:%d).", 
    ...
    static R read_mode_hook(OriginF fn, const char* fn_name, int flags, SOCKET s, Args && ... args)
    {
        Task *tk = Processer::GetCurrentTask();
        bool is_nonblocking = IsNonblocking(s);
        LIBGO_DebugPrint(dbg_hook, "task(%s) Hook %s(s=%d)(nonblocking:%d)(flags:%d).",
